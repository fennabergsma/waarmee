...
In this section I discuss the forms \tit{met 't} `with it` and \tit{'rmee} `with it'. Below I repeat examples from Section \ref{sec:rdefault} that show that \tit{met} `with` can express instrumental case, and \tit{'t} is an inanimate pronoun. \ref{ex:metdp} shows \tit{met} `with' combined with full DP. \ref{ex:tverb} shows \tit{'t} as the object of verbs.
