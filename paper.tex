...
This paper: \tit{waarmee} is the default realization of an instrumental when all features form a constituent. If not, then \tit{met wat}. The \tit{r} in ʀ-pronouns does not necessarily express \tsc{location}, in \tit{waarmee} is only expresses \tsc{thing}. The change of \tit{met} into \tit{mee} is a reflex of stranded features/a different spellout of features.

Unmarked examples are constructed and have been verified by native speakers.



\section{\tit{\tsc{r}}-pronouns as default \\
Distribution between \tit{waarmee} and \tit{met wat}}
...
What this shows is that ʀ-pronouns are not exceptions, but they are the default. There is something special about the examples in which \tit{met wat} is used. I suggest that the crucial difference lies in constituency. \tit{Waarmee} can only be used if the instrumental PP forms a proper constituent, i.e. a constituent to the exclusion of any other elements. In the examples from xx to xx above this is the case.
