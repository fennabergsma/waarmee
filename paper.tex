...
\ref{ex:summaryconst} summarizes what I showed in this section. \tit{Met wat} `with what' can never surface when \tit{met} `with' and \tit{wat} `what' form a proper constituent. It always becomes \tit{waar-mee} `with what', as shown in \ref{ex:waar-meefr}.
...
In this section I decompose \tit{waar-mee} `with what' and \tit{met wat} `met wat' into smaller units, and I connect them to parts of syntactic structure. Both spell out the same set of features but the distribution is different. I decompose \tit{waar-mee} and \tit{met wat} as I show in \ref{ex:decompose}. Earlier works have decomposed these pronouns in similar ways (cf. \citet{hachem2015,noonan2017dutch,wesseling2018}).
...
Let me start with the morphemes \tit{w} and \tit{a} that appear in both expressions. I assume that they correspond to the same syntactic structure in both \tit{waar-mee} `with what' and \tit{met wat} `with what'. As I am interested in the differencs between the two expression, I do not discuss the featural content of \tit{w} and \tit{a} into depth.
...
In the next section I put all features back together in a derivation and I show how \tit{waar-mee} `with what' surfaces when all features form a constituent. \tit{Met wat} `with what' appears when the functional sequence is disrupted.
...
This section showed how the instrumental inanimate relative pronoun is realized as \tit{waar-mee} `with what' when all syntactic features form a constituent. It also showed how \tit{met wat} `with what' appears when all features do not form a proper constituent, and other features intervene.
